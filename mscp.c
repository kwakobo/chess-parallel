...
static void print_move_long(int move, byte *board)
{
...
static unsigned long compute_hash(byte *board)
{
...
static void reset(byte *board)
{
        move_sp = move_stack;
        undo_sp = undo_stack;
        hash_stack[ply] = compute_hash(board);
}

static void setup_board(char *fen, byte *board)
{
...
static void atk_slide(int sq, byte dirs, struct side *s, byte *board)
{
        byte dir = 0;
        int to;
...
static void compute_attacks(byte *board)
{
        int sq, to, pc;
        byte dir, dirs;
        ...
        for (sq=0; sq<64; sq++) {
                ...
                case WHITE_QUEEN:
                        atk_slide(sq, ATK_SLIDER, &white, board);
                        break;

                case BLACK_QUEEN:
                        atk_slide(sq, ATK_SLIDER, &black, board);
                        break;

                case WHITE_ROOK:
                        atk_slide(sq, ATK_ORTHOGONAL, &white, board);
                        break;

                case BLACK_ROOK:
                        atk_slide(sq, ATK_ORTHOGONAL, &black, board);
                        break;

                case WHITE_BISHOP:
                        atk_slide(sq, ATK_DIAGONAL, &white, board);
                        break;

                case BLACK_BISHOP:
                        atk_slide(sq, ATK_DIAGONAL, &black, board);
                        break;
...
static void unmake_move(byte *board)
{
        int sq;
...
static void make_move(int move, byte *board)
{
        int fr;
        int to;
        int sq;
        ...
        *undo_sp++ = -1;                        /* Place sentinel */
        ...
        if (move & SPECIAL) {                   /* Special moves first */
                ...
                case RANK_8:                    /* Black castles */
                        unmake_move(board);
                        if (to == G8) {
                                make_move(MOVE(H8,F8), board);
                        } else {
                                make_move(MOVE(A8,D8), board);
                        }
                        break;
                ...
                case RANK_1:                    /* White castles */
                        unmake_move(board);
                        if (to == G1) {
                                make_move(MOVE(H1,F1), board);
                        } else {
                                make_move(MOVE(A1,D1), board);
                        }
                        break;
...
static int push_move(int fr, int to, byte *board)
{
...
static void push_pawn_move(int fr, int to, byte *board)
{
        if ((R(to) == RANK_8) || (R(to) == RANK_1)) {
                ...
                push_move(fr, to, board);
        }
}

static void gen_slides(int fr, byte dirs, byte *board)
{
        int vector;
        int to;
        byte dir = 0;
...
static int test_illegal(int move, byte *board)
{
        make_move(move, board);
        compute_attacks(board);
        unmake_move(board);
        return friend->attack[enemy->king] != 0;
}

static void generate_moves(unsigned treshold, byte *board)
{
        int             fr, to;
        int             pc;
        byte            dir, dirs;
        ...
        for (fr=0; fr<64; fr++) {
                ...
                case WHITE_QUEEN:
                case BLACK_QUEEN:
                        gen_slides(fr, ATK_SLIDER, board);
                        break;

                case WHITE_ROOK:
                case BLACK_ROOK:
                        gen_slides(fr, ATK_ORTHOGONAL, board);
                        break;

                case WHITE_BISHOP:
                case BLACK_BISHOP:
                        gen_slides(fr, ATK_DIAGONAL, board);
                        break;
        ...
        if (board[EP]) {
                ...
                if (WTM) {
                        if (F(ep) != FILE_A && board[ep-DIR_E] == WHITE_PAWN) {
                                if (push_move(ep-DIR_E, ep+DIR_N, board))
                                        move_sp[-1].move |= SPECIAL;
                        }
...
static void print_move_san(int move, byte *board)
{
        int fr, to;
        int filex = 0, rankx = 0;
        struct move *moves;
...
check:
        ...
        if (enemy->attack[friend->king]) {      /* in check, is mate? */
                ...
                while (move_sp > moves) {
                        move_sp--;
                        if (!test_illegal(move_sp->move, board)) {
                                sign = '+';
                                move_sp = moves;        /* break */
                        }
                }
...
static int parse_move(char *line, int *num, byte *board)
{
        int                     move, matches;
        int                     n = 0;
        struct move             *m;
        ...
        *num = n;
        ...
        while (move_sp > m) {
                ...
                if (move) {
                        ...
                        if (piece) {
                                puts("ambiguous!"); print_board(board); puts(line);
                                move_sp = m;
                                return 0;
                        }
...
static void load_book(char *filename, byte *board)
{
        FILE                    *fp;
        char                    line[128], *s;
        int                     num, move;
        ...
        while (readline(line, sizeof(line), fp) >= 0) {
                s = line;
                for (;;) {
                        move = parse_move(s, &num, board);
                        if (!move) break;

                        s += num;
                        if (booksize < CORE) {
                                BOOK[booksize].hash = compute_hash(board);
                ...
                while (ply>0) unmake_move(board); /* @@@ wrong */
        }
        fclose(fp);
        compact_book();
}

static int book_move(byte *board)
{
        int move = 0, sum = 0;
        long x = 0, y, m;
        char *seperator = "book:";
        ...
        while (BOOK[x].hash == hash) {
                ...
                seperator = ",";
                print_move_san(BOOK[x].move, board);
                compute_hash(board);
...
static void compute_piece_square_tables(byte *board)
{
        int pc, sq;
        int score = 0;
        int file;

        compute_attacks(board); /* so we know where the king and pawns are */
...
static int qsearch(int alpha, int beta, byte *board)
{
        int                             best_score;
        int                             score;
        struct move                     *moves;
        ...
        moves = move_sp;
        generate_moves(PRESCORE_EQUAL + (1<<9), board);
...
static int search(int depth, int alpha, int beta, byte *board)
{
        int                             best_score = -INF;
        int                             best_move = 0;
        int                             score;
        ...
        while (move_sp > moves) {
                ...
                if (newdepth <= 0) {
                        ...
                        score = -search(newdepth, -beta, -alpha, board);
                }
...
static int root_search(int maxdepth, byte *board)
{
        int             depth;
        ...
        nodes = 0;
        compute_piece_square_tables(board);
...
/*----------------------------------------------------------------------+
 ...
 +----------------------------------------------------------------------*/
/*
static void cmd_bd(char *dummy, byte *board)
{
        print_board(board);
}

static void cmd_book(char *dummy, byte *board)
{
        int move;
...
static void cmd_list_moves(char *dummy, byte *board)
{
        struct move *m;
        int nmoves = 0;
        ...
        while (move_sp > m) {
                --move_sp;
                if (test_illegal(move_sp->move, board)) continue;
                print_move_san(move_sp->move, board);
                putchar('\n');
                nmoves++;
        }
        printf("%d move%s\n", nmoves, nmoves==1 ? "" : "s");
...
static void cmd_default(char *s, byte *board)
{
        int move, dummy;

        move = parse_move(s, &dummy, board);
        if (move) {
                make_move(move, board);
                hash_stack[ply] = compute_hash(board);
                print_board(board);
...
static void cmd_undo(char *dummy, byte *board)
{
...
static void cmd_white(char *dummy, byte *board)
{
        computer[0] = 0;
        computer[1] = 1;
        ply = 0;
...
static void cmd_black(char *dummy, byte *board)
{
        computer[0] = 1;
        computer[1] = 0;
        ply = 1;
...
static void cmd_test(char *s, byte *board)
{
        int d = maxdepth;
        sscanf(s, "%*s%d", &d);
        root_search(d, board);
...
static void cmd_new(char *dummy, byte *board)
{
        setup_board("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -", board);
        load_book("book.txt", board);
        computer[0] = 0;
        computer[1] = 1;
        rnd_seed = time(NULL);
}
...
static void cmd_fen(char *s, byte *board)
{
        while (xisalpha(*s)) s++;
        setup_board(s, board);
}
        ...
        for (;;) {
                ...
                while (computer[!WTM]) {
                        ...
                        if (!move) {
                                ...
                                move = root_search(maxdepth, board);
                        }
                        if (!move || ply >= 1000) {
                                printf("game over: ");
                                compute_attacks(board);
